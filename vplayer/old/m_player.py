...
class Player:
  ...
  def _processVideo( self, forced= False ):
    a = 0
    while 1:
      # print a, len(self.decodedFrames)
      a = a + 1
      if len( self.decodedFrames )== 0:
        return
      ...
      # print self.vDelay, self.seekADelta, self._getPTS()
      ...
      if self.vDelay> 0 and self._getSndLeft()> self.vDelay:
        # print "Sleeping: ", self.vDelay/6
        time.sleep( self.vDelay / 6 )
        #print 'Delay', self.vDelay, self.getSndLeft()
        self.vDelay= 0
      
      
      # print self.vDelay, float(frRate)/4
      if self.vDelay< float(frRate) / 4 or forced:
